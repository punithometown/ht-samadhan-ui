...
// Interface matching the backend SparePartRequest schema (with populated ticket)
interface SparePartRequest {
  ...
  } | string; // if not populated, just the ID
  sparePart: string;
  quantity: number;
...
const getLocation = (req: SparePartRequest): string => {
  if (typeof req.ticketId === 'object' && req.ticketId) {
    const addr = req.ticketId.serviceAddress;
    if (addr) return `${addr.city || ''} ${addr.state || ''}`.trim() || req.ticketId.site;
    return req.ticketId.site || 'N/A';
  }
  return 'N/A';
};

const getTicketNumber = (req: SparePartRequest): string => {
  if (typeof req.ticketId === 'object' && req.ticketId) {
    return req.ticketId.ticketNumber;
  }
  return 'N/A';
};
...
export const WarehouseRequests: React.FC = () => {
  ...
  // Fetch spare part requests from API
  const fetchRequests = async () => {
    try {
      setLoading(true);
      // Populate ticket details to get site/store info
  ...
  // Extract unique stores from ticket objects (if populated)
  const stores = ['All Stores', ...new Set(
    requests
      .filter(req => typeof req.ticketId === 'object' && req.ticketId?.site)
      .map(req => (req.ticketId as any).site)
      .filter(Boolean)
  )];

  const filteredRequests = requests.filter(req => {
    const store = (typeof req.ticketId === 'object' && req.ticketId?.site) || 'Unknown';
    const matchesStore = selectedStore === 'All Stores' || store === selectedStore;
    const matchesSearch = 
      req.sparePart.toLowerCase().includes(searchQuery.toLowerCase()) ||
      getTicketNumber(req).toLowerCase().includes(searchQuery.toLowerCase()) ||
      (typeof req.ticketId === 'string' && req.ticketId.includes(searchQuery)) ||
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex items-center gap-4">
        ...
        <input 
          type="text" 
          placeholder="Search by Part, Ticket #, or Reason..." 
          value={searchQuery}
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50/50">
              <tr className="text-[10px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-100">
                ...
                <th className="px-6 py-5">Part & SKU</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredRequests.map((req) => {
                const ticketNumber = getTicketNumber(req);
                  ...
                  <tr key={req._id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] px-1.5 py-0.5 bg-slate-900 text-white rounded font-bold">{ticketNumber}</span>
                      </div>
                      <p className="text-[10px] font-bold text-slate-600 mt-1 uppercase tracking-tighter">
                        {typeof req.ticketId === 'object' ? req.ticketId.site : 'N/A'}
                      </p>
                    ...
                    <td className="px-6 py-4">
                      ...
                      <p className="text-[10px] text-slate-400 font-mono mt-0.5">Urgency: {req.urgency}</p>
                      {req.reason && <p className="text-[9px] text-slate-500 mt-1 italic">Reason: {req.reason}</p>}
                    </td>
                    <td className="px-6 py-4">
             ...
             >
                         ...
                         {(['APPROVED', 'DELIVERED', 'REJECTED', 'PENDING'] as SparePartRequest['status'][]).map((status) => (
                            <button 
                              ...
                              className={`p-4 rounded-xl text-[10px] font-bold uppercase tracking-widest border flex flex-col items-center gap-2 transition-all ${
                                selectedRequest.status === status ? 'bg-orange-500 text-white border-orange-600 shadow-lg shadow-orange-500/20' : 'bg-white text-slate-600 border-slate-200 hover:border-orange-300'
                              } disabled:opacity-50`}
                            >
                ...
                <div className="p-6 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between">
                   <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-slate-500 uppercase">{getLocation(selectedRequest)}</span>
                   </div>
                   <button 
                     onClick={() => setSelectedRequest(null)}
